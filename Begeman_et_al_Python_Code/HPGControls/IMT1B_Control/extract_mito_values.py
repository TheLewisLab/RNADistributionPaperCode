...
for file in os.listdir(csv_dir):
    ...
    hpg_int_sum = mito_df['Sum, Intensities #2'].sum()
    hpg_cell_int = float(cell_df['Sum, Intensities #2'].sum() / cell_df['Area (Mesh), Projection (XY/Z) (µm²)'].sum())
...
out_df.to_csv('RNR2_IMT1B_Control_values.csv')